...
-- Multi-fruit UI display
local function formatFruitText(name, dist)
    return string.format("%s - %.1f studs", (name == "Fruit " and "Spawned Fruit" or name), dist)
end

-- Clear previous labels
local function clearFruitLabels()
    for _, child in ipairs(frame:GetChildren()) do
        if child:IsA("TextLabel") and child ~= fruitLabel then
            child:Destroy()
        end
    end
end
    ...
    while true do
        if not isTypingFeedback then
            ...
            for _, fruit in ipairs(Workspace:GetChildren()) do
                if table.find(fruitsList, fruit.Name) and fruit:FindFirstChild("Handle") then
                    local dist = (humanoidRootPart.Position - fruit.Handle.Position).Magnitude
                    table.insert(detected, {fruit = fruit, dist = dist})
                end
            end
            ...
            if #detected > 0 then
                -- Resize the frame
                frame.Size = UDim2.new(0.3, 0, 0.08 * #detected, 0)

                for i, data in ipairs(detected) do
                    local label = Instance.new("TextLabel")
                    label.Size = UDim2.new(1, 0, 1 / #detected, 0)
                    label.Position = UDim2.new(0, 0, (i - 1) / #detected, 0)
                    label.BackgroundTransparency = 1
                    label.TextColor3 = Color3.fromRGB(255, 255, 255)
                    label.TextScaled = true
                    label.Font = Enum.Font.GothamBold
                    label.TextStrokeTransparency = 0
                    label.TextStrokeColor3 = Color3.fromRGB(50, 50, 50)
                    label.Text = formatFruitText(data.fruit.Name, data.dist)
                    label.ZIndex = 2
                    label.Parent = frame
                end
            else
